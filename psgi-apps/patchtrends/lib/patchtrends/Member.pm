...
# match tbl_users
__PACKAGE__->attributes(qw/id username phone email status create_date last_updated expires subscr_id payer_id type interval/);
...
sub load {
    ...
    my $sSQL = qq{SELECT
                  id,username,phone,email,password,status,create_date,last_updated,expires,
                  subscr_id,payer_id,type,last_updated
...
sub save {
    ...
    if ($self->id()) {
        return $self->_do_update($dbh);
    }
    else {
        return $self->_do_create($dbh);
    }
}
...
sub _do_update {
    ...
    my $uSQL = q{UPDATE tbl_users SET username=?,phone=?,email=?,password=?,status=?,create_date=?,last_updated=?,expires=?,subscr_id=?,payer_id=?,type=?,last_updated=now() WHERE id=?};
...
sub _do_create {
    ...
    $self->username($self->_ensure_username($self->username(), $dbh));
...
sub verify_current_password {
  my ($self, $dbh, $password) = @_;
  
  my $sSQL = q{SELECT id, password, email FROM tbl_users WHERE username = ? AND status = 'active' AND now() <= expires};
  my $results = eval {$dbh->selectall_arrayref($sSQL, {Slice => {}}, $self->username)};
